...
fn rewrite_internal(join: Join) -> Result<Transformed<LogicalPlan>> {
    ...
    let new_plan = if let Some(expr_map) = collected_count_expr_map {
        ...
        for (i, (qualifier, field)) in join_schema.iter().enumerate() {
            let name = field.name();
            // The indicator is only needed by the compensation expressions below.
            // Keeping it in the output can collide with an enclosing join's columns.
            if i >= left_field_count && name == &unmatched_row_indicator {
                continue;
            }
