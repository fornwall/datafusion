...
fn build_join(
    ...
    if count_expr_map.is_none() && pull_up.pull_up_having_expr.is_some() {
        // A filter was taken out of the subquery for the join to compensate, but
        // the empty-input values it needs are gone -- a correlated predicate
        // further up discarded them. Evaluating it is then no longer possible
        // and dropping it would silently change the answer, so leave the
        // subquery alone.
        return Ok(None);
    }
...
fn build_count_bug_join(
    ...
    let in_expr = match in_predicate_opt {
        ...
        Some(Expr::BinaryExpr(BinaryExpr {
            ...
            // Every count-bug-affected output of the subquery has its empty-input
            // value collected, so a miss means the compensation would be unsound
            // rather than merely unknown.
            let Some(on_empty) = count_expr_map.get(&value_col.name).cloned() else {
                return internal_err!(
                    "no empty-input value collected for subquery output column {}",
                    value_col.name
                );
            };
            Some(Expr::eq(
