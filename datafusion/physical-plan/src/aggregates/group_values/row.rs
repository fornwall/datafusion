...
impl GroupValues for GroupValuesRows {
    fn intern(&mut self, cols: &[ArrayRef], groups: &mut Vec<usize>) -> Result<()> {
        ...
        // totalOrder) and primitive hashing group each of them together.
        // No-op for non-float columns.
        let canonicalized_cols: Vec<ArrayRef> =
            cols.iter().map(canonicalize_floats).collect();
        let cols = canonicalized_cols.as_slice();
